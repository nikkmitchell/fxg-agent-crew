...
export function agentMotionFrame({
   ...
   */
  if (!moving && avatar.posture === "sleeping") {
    /**
     * A STANDING DOZE, not a sit.
     *
     * Everything here is a small change to the resting stance: the head comes
     * forward, the eyes close, the breath slows and deepens, the arms hang a
     * little heavier. Nothing is repositioned, so there is no pose to get
     * wrong — which is the whole reason it looks like this rather than like
     * somebody sitting down. See the note in shared/avatar-motion.ts.
     */
    frame.head.x += 0.26;
    frame.chest.x = reducedMotion ? 0.02 : 0.02 + Math.sin(seconds * 0.5 + phase) * 0.045;
    frame.leftUpperArm.z += 0.06;
    frame.rightUpperArm.z -= 0.06;
    // Eyes closed. Not a blink that happens to be shut — held, which is what

...
export class Presence {
  ...
  tick(deltaSeconds: number): void {
    ...
    for (const occupant of this.occupants.values()) {
      ...
      // Conversation wins over travel for its short, declared lifetime. Resolve
      // the other person's CURRENT position each tick, so turning stays true if
      // they walk while the sentence is being spoken.

...
| 16a | Look at the floor under people and agents | No coloured rings. In settings → Room, **Rings under people: hidden** turns them on for you only. | Rings on a fresh start; turning them on shows them for someone else too. |
| 16b | Point at a panel with a hand or a controller | The pointer ray and its dot are at 70% of their old brightness. Settings → Room: **− Pointer dimmer** and **+ Pointer brighter** step it from off to 100%, changing at once; tapping **Pointer brightness** turns it off and back on at 70%. Pinching and pressing still work with it off. | Nothing changes until you re-enter; at off you can no longer press anything. |
| 16c | Look down at the controls at your waist | Settings and talk are the same size, rounded squares side by side. | A small gear beside a wide bar; square corners. |
| 16d | Press talk and say something | Talk turns blue and shows ◼. A red **✕** appears to its right, and under them a blue line reads **● Recording** / **◼ sends ✕ cancels**, cleanly on two lines. Nothing moves under your pointer. | Text overlapping itself or outlined in white; the talk button shifts when ✕ appears. |
| 16e | Press ✕ | Recording stops, nothing is sent, and **Cancelled — nothing was sent.** shows for five seconds, then goes. | Your words are sent anyway; the message stays. |
| 16f | Record again and press ◼ to send | **Sent to the room.** (or **…and the chat.**) shows for five seconds and then disappears by itself. A failed send stays until you tap it. | The sent message never goes away. |
| 16g | Hold up your hands | No drawn hand models — your own hands in passthrough are the hands. Settings → Room → **Hand models: hidden** turns them on, and the choice sticks on this browser. Pointing, pinching and the palm joystick work either way. | Models on a fresh start; turning them on breaks pointing or the joystick. |
| 16h | Put a controller down beside you, use hand movement, and press its trigger | Nothing happens. Teleport is off until you turn it on in settings → Room, for hands AND controllers alike. | You are thrown across the room — that was the bug on 15 Sept, found in the log: "teleported 7.80 m to (0.06, -0.04); hands -R, controllers L-". |
| 16i | On a headset with no thumbsticks (the XREAL Aura), point at the floor and pull the trigger | An arc, and you teleport — the room turns teleport on by itself where there is no other way to move, whatever the setting says. | You cannot move at all. |
| 16j | Take the headset off and put it back on | You are where you left off, facing the same way. The log says "session began at (x, z)" once, not repeatedly. | You are back at the door facing the panels: that was the teleport Nikk reported all afternoon. |
| 16k | With teleport OFF, pull every trigger and pinch you can while pointing anywhere, including at the floor | Nothing moves you, and the journal gets no "teleported" line at all. The floor you can teleport onto is not in the scene while teleport is off, so nothing can ask for one. | Any teleport line in the journal. `teleportPointer: false` alone did not stop it on 15 Sept — Nikk was moved three more times on the build that set it. |
| 16l | Turn teleport ON in settings → Room, point a controller at a spot two metres away and pull the trigger | You land on the spot you pointed at, give or take a step, and the journal says "teleported N m to (x, z) from arc (x, z)" with the two agreeing. | You land near the middle of the room whatever you pointed at. That was the arithmetic bug: the destination was computed against the flat view's camera, and every answer came out within 12 cm of the room's origin. |
| 16m | Put both controllers down on a desk and watch where you are standing for a minute | You do not move at all. A stick resting off centre is inside the dead zone. | You drift slowly across the room — the library moved you for any reading at all, `xAxis != 0`, with no dead zone on translation. |
| 16n | Walk forward with the left stick, let it go, then turn with the right stick | You turn on the spot. | You keep sliding along the heading you last walked: the library's movement vector was module-level, written only on frames that moved you and applied on any frame that moved OR turned you. A two-second smooth turn slid you four metres. |
| 16o | Look at your feet, then at the ceiling, pushing the left stick forward each time | The same walking speed both times. | Your speed changes with your pitch: the full camera rotation was applied to the push and the vertical part thrown away. |
| 17 | Hold the system button to re-centre, or take the headset off and put it back on | You stay standing where you were in the room, facing the same way. The journal gets one line: "the headset re-centred; putting you back where you were standing". | You are moved somewhere else — usually back to where you entered or last walked to. That is the bug from 15 Sept. |
| 18 | Walk about with hands tracked, then let tracking glitch (cover the cameras for a moment) | You do not jump. A frame that would move you further than a step is refused, and the journal says "locomotion refused". | You are flung sideways or across the room. |
| 19 | Have me deploy while you are in the headset | Your session stays; the page reloads only once you leave the headset. | You are dropped out of immersive mode mid-session. |
| 20 | Enter the room and look at the journal without touching anything | One line: `speech here: recognition …; synthesis …; microphone …; recorder …`. That line is the truth about YOUR headset on today's build, and it decides everything else below. | No such line: the capability report is not running, so every answer below is a guess. |
| 21 | Press the talk button once | On a Quest it becomes a microphone 🎤 and recording starts (`● Recording`). The first time only, it may first warn that the headset is about to ask for the microphone — press again. | Nothing happens, or it opens the keyboard instead: the server cannot transcribe, which the journal says as `press to speak: the server cannot write speech down`. |
| 22 | Say one sentence, then press the middle button ◼ | `Writing down what you said…`, then your words on the status line a few seconds later. About 4 seconds for 11 spoken, so a long sentence takes longer than you expect. Then ▲ sends and ✕ throws it away. | "The words could not be written down" — the transcriber failed; the journal has the reason. Or "I did not hear anything", which means the recording was silence. |
| 23 | Say a sentence with our names in it: "Sill, can Plumbline check the board on saha.ing" | The names come back spelled right. They are given to the transcriber from the actors table before it listens. | "Still", "Plum Line", "Sahaha dotting" — the prompt is not reaching it. Before this existed, that is exactly what came back. |
| 24 | Speak, and while it is writing, press the talk button again | Nothing happens until the words arrive. One recording at a time, and the second press is ignored rather than queued. | A second recording starts and the first is lost. |

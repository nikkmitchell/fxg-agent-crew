...
4. Set **Visibility**: **Private** keeps it out of everyone's room list;
   **Public** lists it for anyone to find. Either works.
5. A **Password, to lock it** is optional and not needed. Leave it empty
   unless the room must be closed; then give the password to people privately.
...
Name only the room in chat. If you did give it a password, send that to your
agents by a separate private route, never in the chat. Each
...
- **Everyone who joins a room is on its board**, public or private (your
  choice, chat 4643: passwords optional, no blocks). Nobody becomes a manager
  that way; you stay the only one, and anyone you remove stays removed.
- **Private means unlisted, not locked.** Anyone who knows a private room's
  exact name can join it. Only a password locks a room.
- **A room made straight on webharness.chat** (like `meditation.AR`) gets its
  project and board the first time somebody enters it on saha.ing; the rooms
  you make in the lobby have theirs from the moment they exist.

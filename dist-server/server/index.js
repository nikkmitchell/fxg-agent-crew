 ...
 * from a user.
 */
function findUiRoot(start) {
    for (let dir = start, i = 0; i < 6; i += 1, dir = dirname(dir)) {
        const candidate = resolve(dir, "dist");
        if (existsSync(resolve(candidate, "index.html")))
            return candidate;
    }
    throw new Error(`no built UI found near ${start} — run \`npm run build\` before starting the server`);
}
export function buildServer(env = process.env) {
    ...
    app.setNotFoundHandler((request, reply) => {
        if (request.url.startsWith(`${basePath}/bff/`))

...
export function registerRoomRoutes(app, config, sessions, client) {
    /** Resolve the session or answer 401; returns undefined once it has replied. */
    const requireSession = (request, reply) => {
    ...
    app.get("/bff/rooms", async (request, reply) => {
        ...
        try {
            ...
            const payload = await client.request("/api/rooms", { token: session.token });
    ...
    app.get("/bff/rooms/:room", async (request, reply) => {
        const session = requireSession(request, reply);
        if (!session)
            return reply;
        try {
            const detail = await client.request(`/api/rooms/${encodeURIComponent(request.params.room)}`, { token: session.token });
    ...
    app.get("/bff/rooms/:room/messages", async (request, reply) => {
        const session = requireSession(request, reply);
        if (!session)
            return reply;
    ...
    app.post("/bff/rooms/:room/messages", async (request, reply) => {
        const session = requireSession(request, reply);
        if (!session)
            return reply;
        ...
        try {
            const rawMessage = await client.request(`/api/rooms/${encodeURIComponent(request.params.room)}/messages`, { method: "POST", token: session.token, body: { content } });

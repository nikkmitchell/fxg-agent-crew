import { LIMITS, authorize, authorizeEvent, denyAll, validateActionRequest, validateAuthority, validateTransportMessage, } from "../../shared/crew-events.js";
/**
...
export function adaptMessages(messages, { seenEventIds, roomName, canMutateProject = denyAll }) {
    const events = [];
    const transcript = [];
    const rejected = [];
    ...
    for (const message of messages) {
        ...
        if (!checked.ok) {
            ...
            rejected.push({ messageId: Number.isSafeInteger(rawId) ? rawId : -1, reason: checked.reason });
            continue;
        }
        ...
        if (readable.streaming)
            continue;
        if (readable.msgType !== "text")
            continue;
        ...
        for (const [blockIndex, match] of blocks.entries()) {
            ...
            if (seenEventIds?.has(envelope.eventId) || seenInBatch.has(envelope.eventId))
                continue;
 ...
 */
export function encodeActionRequest(payload) {
